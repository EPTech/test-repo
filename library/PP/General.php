...
// Redirect to another page or site
  static function  zen_redirect($url) {
    global $logger;
...
// Parse the data used in the html tags to ensure the tags will not break
  static function  zen_parse_input_field_data($data, $parse) {
    return strtr(trim($data), $parse);
  }


  static function  zen_output_string($string, $translate = false, $protected = false) {
    if ($protected == true) {
      return htmlspecialchars($string, ENT_COMPAT, CHARSET, FALSE);
    } else {
      if ($translate == false) {
        return PP_General::zen_parse_input_field_data($string, array('"' => '&quot;'));
      } else {
        return PP_General::zen_parse_input_field_data($string, $translate);
      }
    }
  }


  static function  zen_output_string_protected($string) {
   PP_General::zen_output_string($string, false, true);
  }


  static function  zen_sanitize_string($string) {
  ...
  static function  zen_customers_name($customers_id) {
    global $db;
  ...
  static function  zen_get_path($current_category_id = '') {
...
// set to 0 if Top Level
  ...
  static function  zen_get_all_get_params($exclude_array = '') {
    global $_GET;
  ...
  static function  zen_date_long($raw_date) {
...
// NOTE: Includes a workaround for dates before 01/01/1970 that fail on windows servers
  static function  zen_date_short($raw_date) {
    if ( ($raw_date == '0001-01-01 00:00:00') || ($raw_date == '') ) return false;
...
// error on 1969 only allows for leap year
  ...
  static function  zen_datetime_short($raw_datetime) {
  ...
  static function  zen_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false, $category_has_products = false, $limit = false) {
    global $db;
...
// products with name, model and price pulldown
  static function  zen_draw_products_pull_down($name, $parameters = '', $exclude = '', $show_id = false, $set_selected = false, $show_model = false, $show_current_category = false) {
...
// only show $current_categories_id
  ...
  static function  zen_options_name($options_id) {
    global $db;
  ...
  static function  zen_values_name($values_id) {
    global $db;
  ...
  static function  zen_info_image($image, $alt, $width = '', $height = '') {
  ...
  static function  zen_break_string($string, $len, $break_char = '-') {
    $l = 0;
    $output = '';
  ...
  static function  zen_get_country_name($country_id) {
    global $db;
  ...
  static function  zen_get_country_name_cfg() {
    global $db;
    $country = $db->Execute("select countries_name
                             from " . TABLE_COUNTRIES . "
                             where countries_id = '" . (int)$country_id . "'");
  ...
  static function  zen_get_zone_name($country_id, $zone_id, $default_zone) {
    global $db;
  ...
  static function  zen_not_null($value) {
    if (is_array($value)) {
      if (sizeof($value) > 0) {
        return true;
      } else {
  ...
  static function  zen_browser_detect($component) {

    return stristr($_SERVER['HTTP_USER_AGENT'], $component);
  }


  static function  zen_tax_classes_pull_down($parameters, $selected = '') {
    global $db;
  ...
  static function  zen_geo_zones_pull_down($parameters, $selected = '') {
    global $db;
    $select_string = '<select ' . $parameters . '>';
  ...
  static function  zen_get_geo_zone_name($geo_zone_id) {
    global $db;
...
/*
  static function  zen_address_format($address_format_id, $address, $html, $boln, $eoln) {
    global $db;
...
*/
  ...
  //
  ////////////////////////////////////////////////////////////////////////////////////////////////
  static function  zen_get_zone_code($country_id, $zone_id, $default_zone) {
    global $db;
  ...
  static function  zen_get_uprid($prid, $params) {
    $uprid = $prid;
  ...
  static function  zen_get_prid($uprid) {
  ...
  static function  zen_get_languages() {
    global $db;
  ...
  static function  zen_get_category_name($category_id, $language_id) {
    global $db;
  ...
  static function  zen_get_category_description($category_id, $language_id) {
    global $db;
  ...
  static function  zen_get_orders_status_name($orders_status_id, $language_id = '') {
    global $db;
  ...
  static function  zen_get_orders_status() {
    global $db;
  ...
  static function  zen_get_products_name($product_id, $language_id = 0) {
    global $db;
  ...
  static function  zen_get_products_description($product_id, $language_id) {
    global $db;
  ...
  static function  zen_get_products_url($product_id, $language_id) {
    global $db;
...
// TABLES: manufacturers_info
  static function  zen_get_manufacturer_url($manufacturer_id, $language_id) {
    global $db;
...
// This function is not available in all PHP versions so we test it before using it.
  static function  zen_class_exists($class_name) {
...
// TABLES: products, products_to_categories, categories
  static function  zen_products_in_category_count($categories_id, $include_deactivated = false, $include_child = true, $limit = false) {
    global $db;
    $products_count = 0;
...
// TABLES: categories
  static function  zen_childs_in_category_count($categories_id) {
    global $db;
    $categories_count = 0;
...
// TABLES: countries
   ...
   // global $db;
    ...
    $countries = $db->fetchAll("select countries_id, countries_name
                               from " . 'countries' . "
                               order by countries_name");
foreach($countries as $result)
{
      $countries_array[] = array('id' => $result['countries_id'],
...
// return an array with country zones
  static function  zen_get_country_zones($country_id) {
    $db = Zend_db_Table::getDefaultAdapter();
    $zones_array = array();
    $zones = $db->fetchAll("select zone_id, zone_name
                           from " .'zones' . "
                           where zone_country_id = '" . (int)$country_id . "'
                           order by zone_name");

    foreach ($zones as $result) {
      $zones_array[] = array('id' => $result['zone_id'],
                             'text' => $result['zone_name']);
     // $zones->MoveNext();
    }
  ...
  static function  zen_prepare_country_zones_pull_down($country_id = '') {
...
// Get list of address_format_id's
  static function  zen_get_address_formats() {
    global $db;
...
////
  static function  zen_cfg_select_coupon_id($coupon_id, $key = '') {
   $db = Zend_db_Table::getDefaultAdapter();
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
    $coupons = $db->fetchAll("select cd.coupon_name, c.coupon_id from " . 'coupons' ." c, ". ' coupons_description '. " cd where cd.coupon_id = c.coupon_id and cd.language_id = '" . (int)$_SESSION['languages_id'] . "'");
    $coupon_array[] = array('id' => '0',
                            'text' => 'None');

    foreach($coupons as $result) {
      $coupon_array[] = array('id' => $result['coupon_id'],
                              'text' => $result['coupon_name']);
     // $coupons->MoveNext();
    }
   return PP_General::zen_draw_pull_down_menu($name, $coupon_array, $coupon_id);
  }


////
// Alias function for Store configuration values in the Administration Tool
  static function  zen_cfg_pull_down_country_list($country_id, $key = '') {
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
    return PP_General::zen_draw_pull_down_menu($name, PP_General::zen_get_countries(), $country_id);
  }


////
  static function  zen_cfg_pull_down_country_list_none($country_id, $key = '') {
    $country_array = zen_get_countries('None');
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   PP_General::zen_draw_pull_down_menu($name, $country_array, $country_id);
  }


////
  static function  zen_cfg_pull_down_zone_list($zone_id, $key = '') {
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
    return PP_General::zen_draw_pull_down_menu($name, PP_General::zen_get_country_zones(STORE_COUNTRY), $zone_id);
  }


////
  static function  zen_cfg_pull_down_tax_classes($tax_class_id, $key = '') {
    global $db;
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   ...
   PP_General::zen_draw_pull_down_menu($name, $tax_class_array, $tax_class_id);
  }


////
// Function to read in text area in admin
 static function  zen_cfg_textarea($text, $key = '') {
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   PP_General::zen_draw_textarea_field($name, false, 60, 5, htmlspecialchars($text));
  }


////
// Function to read in text area in admin
 static function  zen_cfg_textarea_small($text, $key = '') {
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   PP_General::zen_draw_textarea_field($name, false, 35, 1, htmlspecialchars($text));
  }


  static function  zen_cfg_get_zone_name($zone_id) {
    global $db;
    $zone = $db->Execute("select zone_name
                          from " . TABLE_ZONES . "
  ...
  static function  zen_cfg_pull_down_htmleditors($html_editor, $key = '') {
    global $editors_list;
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   ...
   PP_General::zen_draw_pull_down_menu($name, $editors_pulldown, $html_editor);
  }

  static function  zen_cfg_password_input($value, $key = '') {
   PP_General::zen_draw_password_field('configuration[' . $key . ']', $value);
  }

  static function  zen_cfg_password_display($value) {
    $length = strlen($value);
...
// Sets the status of a product
  static function  zen_set_product_status($products_id, $status) {
    global $db;
    if ($status == '1') {
...
// Cant be used in safe mode.
  static function  zen_set_time_limit($limit) {
...
////
// Alias function for Store configuration values in the Administration Tool
  static function  zen_cfg_select_option($select_array, $key_value, $key = '') {
    $string = '';

    for ($i=0, $n=sizeof($select_array); $i<$n; $i++) {
      $name = ((PP_General::zen_not_null($key)) ? 'configuration[' . $key . ']' : 'configValue');
      if($string != ""){ $string.= "<br/>"; }
  ...
  static function  zen_cfg_select_drop_down($select_array, $key_value, $key = '') {
    $string = '';

    $name = ((PP_General::zen_not_null($key)) ? 'configuration[' . $key . ']' : 'configValue');
  return  PP_General::zen_draw_pull_down_menu($name, $select_array, (int)$key_value);
  }

////
// Alias function for module configuration keys
  static function  zen_mod_select_option($select_array, $key_name, $key_value) {
    reset($select_array);
...
// Retreive server information
  static function  zen_get_system_information() {
    global $db;
  ...
  static function  zen_generate_category_path($id, $from = 'category', $categories_array = '', $index = 0) {
    global $db;
  ...
  static function  zen_output_generated_category_path($id, $from = 'category') {
...
//        $calculated_category_path_string .= $calculated_category_path[$i][$j]['text'] . '&nbsp;&gt;&nbsp;';
  ...
  static function  zen_get_generated_category_path_ids($id, $from = 'category') {
    global $db;
    $calculated_category_path_string = '';
    $calculated_category_path = zen_generate_category_path($id, $from);
    for ($i=0, $n=sizeof($calculated_category_path); $i<$n; $i++) {
  ...
  static function  zen_remove_category($category_id) {
...
//die('DONE TESTING');
  ...
  static function  zen_remove_product($product_id, $ptc = 'true') {
    global $db;
...
//    }
  ...
  static function  zen_products_attributes_download_delete($product_id) {
    global $db;
  ...
  static function  zen_remove_order($order_id, $restock = false) {
    global $db;
    if ($restock == 'on') {
  ...
  static function  zen_get_file_permissions($mode) {
// determine type
...
// adjust for SUID, SGID and sticky bit
  ...
  static function  zen_remove($source) {
...
// Output the tax percentage with optional padded decimals
  static function  zen_display_tax_value($value, $padding = TAX_DECIMAL_PLACES) {
    if (strpos($value, '.')) {
      $loop = true;
      while ($loop) {
        if (substr($value, -1) == '0') {
  ...
  static function  zen_get_tax_class_title($tax_class_id) {
    global $db;
    if ($tax_class_id == '0') {
      return TEXT_NONE;
    } else {
  ...
  static function  zen_banner_image_extension() {
  ...
  static function  zen_round($value, $precision) {
...
// Add tax to a products price
  static function  zen_add_tax($price, $tax) {
    global $currencies;

    if (DISPLAY_PRICE_WITH_TAX_ADMIN == 'true') {
     PP_General::zen_round($price, $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']) + zen_calculate_tax($price, $tax);
    } else {
     PP_General::zen_round($price, $currencies->currencies[DEFAULT_CURRENCY]['decimal_places']);
    }
  }

// Calculates Tax rounding the result
  static function  zen_calculate_tax($price, $tax) {
...
// TABLES: tax_rates, zones_to_geo_zones
  static function  zen_get_tax_rate($class_id, $country_id = -1, $zone_id = -1) {
    global $db;
...
// TABLES: tax_rates
  static function  zen_get_tax_rate_value($class_id) {
   PP_General::zen_get_tax_rate($class_id);
  }

  static function  zen_call_function($function, $parameter, $object = '') {
    if ($object == '') {
  ...
  static function  zen_get_zone_class_title($zone_class_id) {
    global $db;
    if ($zone_class_id == '0') {
      return TEXT_NONE;
    } else {
...
////
  static function  zen_cfg_pull_down_zone_classes($zone_class_id, $key = '') {
    global $db;
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
   ...
   PP_General::zen_draw_pull_down_menu($name, $zone_class_array, $zone_class_id);
  }


////
  static function  zen_cfg_pull_down_order_statuses($order_status_id, $key = '') {
     $db = Zend_db_Table::getDefaultAdapter();
    $name = (($key) ? 'configuration[' . $key . ']' : 'configValue');
    $_SESSION['languages_id']  = 1;
 //   $statuses_array = array(array('id' => '0', 'text' => TEXT_DEFAULT));
      $statuses_array = array();
    $statuses = $db->fetchAll("select orders_status_id, orders_status_name
                              from " .'orders_status' . "
                              where language_id = '" . (int)$_SESSION['languages_id'] . "'
                              order by orders_status_id");

    foreach($statuses as $result) {
      $statuses_array[] = array('id' => $result['orders_status_id'],
                                'text' => $result['orders_status_name'] . ' [' . $result['orders_status_id'] . ']');
     // $statuses->MoveNext();
    }

  return  PP_General::zen_draw_pull_down_menu($name, $statuses_array, $order_status_id);
  }

  static function  zen_get_order_status_name($order_status_id, $language_id = '') {
    global $db;
...
// Return a random value
  static function  zen_rand($min = null, $max = null) {
    static $seeded;
...
// nl2br() prior PHP 4.2.0 did not convert linefeeds on all OSs (it only converted \n)
  static function  zen_convert_linefeeds($from, $to, $string) {
  ...
  static function  zen_string_to_int($string) {
    return (int)$string;
  }

////
// Parse and secure the cPath parameter values
  static function  zen_parse_category_path($cPath) {
...
// Update the Customers GV account
  static function  zen_gv_account_update($customer_id, $gv_id) {
    global $db;
...
// Output a day/month/year dropdown selector
  static function  zen_draw_date_selector($prefix, $date='') {
    $month_array = array();
...
// Validate Option Name and Option Type Match
  static function  zen_validate_options_to_options_value($products_options_id, $products_options_values_id) {
    global $db;
...
// look-up Attributues Options Name products_options_values_to_products_options
  static function  zen_get_products_options_name_from_value($lookup) {
    global $db;
...
// lookup attributes model
  static function  zen_get_products_model($products_id) {
    global $db;
...
// Check if product has attributes
  static function  zen_has_product_attributes_OLD($products_id) {
    global $db;
...
////
// Check if product has attributes
  static function  zen_has_product_attributes($products_id, $not_readonly = 'true') {
    global $db;
...
// Check if product_id is valid
  static function  zen_products_id_valid($products_id) {
    global $db;
...
static function  zen_copy_products_attributes($products_id_from, $products_id_to) {
  global $db;
  global $messageStack;
...
////
// warning message
  static function  zen_output_warning($warning) {
...
// uses $tbl = table name, $fld = field name
  ...
  static function  zen_field_type($tbl, $fld) {
    global $db;
...
// uses $tbl = table name, $fld = field name
  static function  zen_field_length($tbl, $fld) {
    global $db;
    $rs = $db->MetaColumns($tbl);
...
// example: zen_set_field_length(TABLE_CATEGORIES_DESCRIPTION, 'categories_name')
  static function  zen_set_field_length($tbl, $fld, $max=50, $override=false) {
...
// Lookup Languages Icon
  static function  zen_get_language_icon($lookup) {
    global $db;
...
// Get the Option Name for a particular language
  static function  zen_get_option_name_language($option, $language) {
    global $db;
...
////
// Get the Option Name for a particular language
  static function  zen_get_option_name_language_sort_order($option, $language) {
    global $db;
...
////
// lookup attributes model
  static function  zen_get_language_name($lookup) {
    global $db;
...
// Delete all product attributes
  static function  zen_delete_products_attributes($delete_product_id) {
    global $db;
...
// Set Product Attributes Sort Order to Products Option Value Sort Order
  static function  zen_update_attributes_products_option_values_sort_order($products_id) {
    global $db;
...
// product pulldown with attributes
  static function  zen_draw_products_pull_down_attributes($name, $parameters = '', $exclude = '') {
...
// categories pulldown with products
  static function  zen_draw_products_pull_down_categories($name, $parameters = '', $exclude = '', $show_id = false, $show_parent = false) {
    global $db, $currencies;
...
// categories pulldown with products with attributes
  static function  zen_draw_products_pull_down_categories_attributes($name, $parameters = '', $exclude = '') {
    global $db, $currencies;
  ...
  static function  zen_get_top_level_domain($url) {
    if (strpos($url, '://')) {
      $url = parse_url($url);
      $url = $url['host'];
    }
...
// Check if a demo is active
  static function  zen_admin_demo() {
    return (ADMIN_DEMO == '1') ? TRUE : FALSE;
  }

////
//
  static function  zen_has_product_attributes_downloads($products_id, $check_valid=false) {
    global $db;
...
// does not validate download filename
  static function  zen_has_product_attributes_downloads_status($products_id) {
    global $db;
    if (DOWNLOAD_ENABLED == 'true') {
      $download_display_query_raw ="select pa.products_attributes_id, pad.products_attributes_filename
                                    from " . TABLE_PRODUCTS_ATTRIBUTES . " pa, " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " pad
...
// TABLES: products_to_categories
  static function  zen_get_product_path($products_id, $status_override = '1') {
    global $db;
    $cPath = '';
...
// TABLES: categories
  static function  zen_get_parent_categories(&$categories, $categories_id) {
    global $db;
...
// TABLES: products_to_categories
  static function  zen_get_products_category_id($products_id) {
    global $db;
...
// old name zen_get_parent_category_name
  static function  zen_get_products_master_categories_name($categories_id) {
    global $db;
...
// configuration key value lookup
  static function  zen_get_configuration_key_value($lookup) {
    global $db;
    $configuration_query= $db->Execute("select configValue from " . TABLE_CONFIGURATION . " where configuration_key='" . zen_db_input($lookup) . "'");
    $lookup_value= $configuration_query->fields['configValue'];
...
////
// enable shipping
  static function  zen_get_shipping_enabled($shipping_module) {
  ...
  static function  zen_get_handler_from_type($product_type) {
    global $db;
...
// Sets the status of a featured product
  static function  zen_set_featured_status($featured_id, $status) {
    global $db;
    if ($status == '1') {
...
// Sets the status of a product review
  static function  zen_set_reviews_status($review_id, $status) {
    global $db;
    if ($status == '1') {
...
// set the products_price_sorter
  static function  zen_update_products_price_sorter($product_id) {
    global $db;
...
// configuration key value lookup in TABLE_PRODUCT_TYPE_LAYOUT
  static function  zen_get_configuration_key_value_layout($lookup, $type=1) {
    global $db;
    $configuration_query= $db->Execute("select configValue from " . TABLE_PRODUCT_TYPE_LAYOUT . " where configuration_key='" . zen_db_input($lookup) . "' and product_type_id='". (int)$type . "'");
    $lookup_value= $configuration_query->fields['configValue'];
    if ( !($lookup_value) ) {
      $lookup_value='<span class="lookupAttention">' . $lookup . '</span>';
    }
...
// TABLES: categories
  static function  zen_has_category_subcategories($category_id) {
    global $db;
...
////
  static function  zen_get_categories($categories_array = '', $parent_id = '0', $indent = '') {
    global $db;
...
// Get the status of a category
  static function  zen_get_categories_status($categories_id) {
    global $db;
...
// Get the status of a product
  static function  zen_get_products_status($product_id) {
    global $db;
...
////
// check if linked
  static function  zen_get_product_is_linked($product_id, $show_count = 'false') {
    global $db;
...
// TABLES: categories_name from products_id
  static function  zen_get_categories_name_from_product($product_id) {
    global $db;
...
//    $check_products_category= $db->Execute("select products_id, categories_id from " . TABLE_PRODUCTS_TO_CATEGORIES . " where products_id='" . $product_id . "' limit 1");
  ...
  static function  zen_count_products_in_cats($category_id) {
    global $db;
...
// syntax for linked products: zen_get_products_to_categories($categories->fields['categories_id'], true, 'products_active')
  static function  zen_get_products_to_categories($category_id, $include_inactive = false, $counts_what = 'products') {
    global $db;
...
// master category selection
  static function  zen_get_master_categories_pulldown($product_id) {
    global $db;
...
////
// get products type
  static function  zen_get_products_type($product_id) {
    global $db;
...
// USPS Methods 2.5
// Alias function for Store configuration values in the Administration Tool
  static function  zen_cfg_select_multioption($select_array, $key_value, $key = '') {
...
////
// get products image
  static function  zen_get_products_image($product_id) {
    global $db;
    $product_image = $db->Execute("select products_image
                                   from " . TABLE_PRODUCTS . "
                                   where products_id = '" . (int)$product_id . "'");
...
// remove common HTML from text for display as paragraph
  static function  zen_clean_html($clean_it) {
...
// find template or default file
  static function  zen_get_file_directory($check_directory, $check_file, $dir_only = 'false') {
    global $template_dir;
...
// Recursive algorithim to restrict all sub_categories to a rpoduct type
  static function  zen_restrict_sub_categories($zf_cat_id, $zf_type) {
    global $db;
...
////
// Recursive algorithim to restrict all sub_categories to a rpoduct type
  static function  zen_remove_restrict_sub_categories($zf_cat_id, $zf_type) {
    global $db;
    $zp_sql = "select categories_id from " . TABLE_CATEGORIES . " where parent_id = '" . (int)$zf_cat_id . "'";
    $zq_sub_cats = $db->Execute($zp_sql);
    while (!$zq_sub_cats->EOF) {
...
// NOTE: keys are looked up first in the product_type_layout table and if not found looked up in the configuration table.
    static function  zen_get_show_product_switch($lookup, $field, $suffix= 'SHOW_', $prefix= '_INFO', $field_prefix= '_', $field_suffix='') {
      global $db;
      ...
      $sql = "select configuration_key, configValue from " . TABLE_PRODUCT_TYPE_LAYOUT . " where configuration_key='" . zen_db_input($zv_key) . "'";
      $zv_key_value = $db->Execute($sql);
//echo 'I CAN SEE - look ' . $lookup . ' - field ' . $field . ' - key ' . $zv_key . ' value ' . $zv_key_value->fields['configValue'] .'<br>';

      if ($zv_key_value->RecordCount() > 0) {
        return $zv_key_value->fields['configValue'];
      } else {
        $sql = "select configuration_key, configValue from " . TABLE_CONFIGURATION . " where configuration_key='" . zen_db_input($zv_key) . "'";
        $zv_key_value = $db->Execute($sql);
        if ($zv_key_value->RecordCount() > 0) {
          return $zv_key_value->fields['configValue'];
        } else {
...
////
// return switch name
    static function  zen_get_show_product_switch_name($lookup, $field, $suffix= 'SHOW_', $prefix= '_INFO', $field_prefix= '_', $field_suffix='') {
      global $db;
...
// compute the days between two dates
  static function  zen_date_diff($date1, $date2) {
...
// check that a download filename exists
  static function  zen_orders_products_downloads($check_filename) {
    global $db;
...
// salemaker categories array
  static function  zen_parse_salemaker_categories($clist) {
...
// update salemaker product prices per category per product
  static function  zen_update_salemaker_product_prices($salemaker_id) {
    global $db;
...
// check if products has discounts
  static function  zen_has_product_discounts($look_up) {
    global $db;
...
//copy discounts from product to another
  static function  zen_copy_discounts_to_product($copy_from, $copy_to) {
    global $db;
...
// TABLES: categories
  static function  zen_get_parent_category_id($product_id) {
    global $db;
...
//  static function  zen_products_lookup($product_id, $what_field = 'products_name', $language = $_SESSION['languages_id']) {
  static function  zen_products_lookup($product_id, $what_field = 'products_name', $language = '') {
    global $db;
  ...
  static function  zen_count_days($start_date, $end_date, $lookup = 'm') {
    if ($lookup == 'd') {
...
// $my_products_id_list = zen_get_categories_products_list($categories_id)
  static function  zen_get_categories_products_list($categories_id, $include_deactivated = false, $include_child = true) {
    global $db;
...
//      echo 'Products ID: ' . $products->fields['products_id'] . '<br>';
  ...
  static function  zen_geo_zones_pull_down_coupon($parameters, $selected = '') {
    global $db;
    $select_string = '<select ' . $parameters . '>';
    $zones = $db->Execute("select geo_zone_id, geo_zone_name
                                 from " . TABLE_GEO_ZONES . "
...
// customer lookup of address book
  static function  zen_get_customers_address_book($customer_id) {
    global $db;
...
// get customer comments
  static function  zen_get_orders_comments($orders_id) {
    global $db;
...
// manufacturers name
  static function  zen_get_products_manufacturers_name($product_id) {
    global $db;
    ...
    static function  zen_user_has_gv_balance($c_id) {
      global $db;
...
// Draw a 1 pixel black line
  function zen_black_line() {
   PP_General::zen_image(DIR_WS_IMAGES . 'pixel_black.gif', '', '100%', '1');
  }

////
// Output a separator either through whitespace, or with an image
  function zen_draw_separator($image = 'pixel_black.gif', $width = '100%', $height = '1') {
   PP_General::zen_image(DIR_WS_IMAGES . $image, '', $width, $height);
  }
...
// Output a function button in the selected language
  function zen_image_button($image, $alt = '', $params = '') {
   ...
   PP_General::zen_image(DIR_WS_LANGUAGES . $_SESSION['language'] . '/images/buttons/' . $image, $alt, '', '', $params);
  }
...
// Output a form checkbox field
  function zen_draw_checkbox_field($name, $value = '', $checked = false, $compare = '', $parameters = '') {
   PP_General::zen_draw_selection_field($name, 'checkbox', $value, $checked, $compare, $parameters);
  }

////
// Output a form radio field
  function zen_draw_radio_field($name, $value = '', $checked = false, $compare = '', $parameters = '') {
   PP_General::zen_draw_selection_field($name, 'radio', $value, $checked, $compare, $parameters);
  }

////
// Output a form textarea field
  function zen_draw_textarea_field($name, $wrap, $width, $height, $text = '~*~*#', $parameters = '', $reinsert_value = true) {
    $field = '<textarea name="' . PP_General::zen_output_string($name) . '" wrap="' . PP_General::zen_output_string($wrap) . '" cols="' . PP_General::zen_output_string($width) . '" rows="' . PP_General::zen_output_string($height) . '"';

    if (PP_General::zen_not_null($parameters)) $field .= ' ' . $parameters;
    ...
    } elseif ($text != '~*~*#' && PP_General::zen_not_null($text)) {
      $field = str_replace('&gt;', '>', $field);
      $field .= $text;
    }
...
static  function zen_draw_pull_down_menu($name, $values, $default = '', $parameters = '', $required = false) {
//    $field = '<select name="' . zen_output_string($name) . '"';
    $field = '<select rel="dropdown" name="' . PP_General::zen_output_string($name) . '"';
    ...
    for ($i=0, $n=sizeof($values); $i<$n; $i++) {
      $field .= '<option value="' . PP_General::zen_output_string($values[$i]['id']) . '"';
      if ($default == $values[$i]['id']) {
        $field .= ' selected="selected"';
      }

      $field .= '>' . PP_General::zen_output_string($values[$i]['text'], array('"' => '&quot;', '\'' => '&#039;', '<' => '&lt;', '>' => '&gt;')) . '</option>' . "\n";
    }
    $field .= '</select>' . "\n";
...
// Hide form elements
  function zen_hide_session_id() {
    ...
    if ( ($session_started == true) && defined('SID') && zen_not_null(SID) ) {
     PP_General::zen_draw_hidden_field(zen_session_name(), zen_session_id());
    }
  }
