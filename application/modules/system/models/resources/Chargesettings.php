...
class Default_Resource_Chargesettings extends PP_Model_Resource_Db_Table_Abstract {

    protected $_name = 'charge_settings';
    protected $_primary = array('charge_id', 'engcat_id');
  
    public function getChargesettingsByChargeId($id) {
        $select = $this->select();
        $select->where('charge_id = ?',$id)->limit(1);
